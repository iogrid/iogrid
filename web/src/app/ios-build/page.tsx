import type { Metadata } from "next";
import Link from "next/link";
import { MarketingShell } from "@/components/marketing/marketing-shell";
import { PageHero } from "@/components/marketing/page-hero";

export const metadata: Metadata = {
  title: "iOS build CI",
  description: "Real Mac runners with Xcode preinstalled. Tart-managed VMs reset between jobs. ~50% cheaper than GitHub Actions Mac minutes.",
};

export default function IosBuildPage() {
  return (
    <MarketingShell>
      <PageHero
        eyebrow="iOS Build CI"
        title="Half-price Xcode CI on native macOS hardware."
        subtitle="Real Mac runners with Xcode preinstalled. Tart-managed VMs reset between jobs. ~50% cheaper than GitHub Actions Mac minutes."
      />
      <section className="border-b border-border">
        <div className="mx-auto max-w-3xl px-6 py-16">
          <h2 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
            Get started
          </h2>
          <p className="mt-6 text-base leading-relaxed text-muted-foreground">
            Push a workflow, pick a Tart image, get a clean build host. Connect via SSH or GitHub Actions runner protocol.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href="/install"
              className="inline-flex items-center justify-center rounded-md bg-foreground px-4 py-2 text-sm font-medium text-background transition-colors hover:bg-foreground/90"
            >
              Become a Mac provider
            </Link>
            <Link
              href="/pricing"
              className="inline-flex items-center justify-center rounded-md border border-border bg-background px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted"
            >
              See pricing
            </Link>
          </div>
        </div>
      </section>
    </MarketingShell>
  );
}
