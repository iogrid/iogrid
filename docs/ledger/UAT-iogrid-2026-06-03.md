...
### TC-09 — Account: sessions + identifiers (recovery surfaces)  🟢 *(both surfaces fixed + tested 2026-06-04)*

- **Goal:** *"As a user I review which devices are signed in and which emails can recover my account."*

| # | Screen | What you do | What you must see | Result | Evidence |
|---|---|---|---|---|---|
| 1 | [/account/sessions](https://iogrid.org/account/sessions) | Open **Sessions** | The signed-in sessions list — at minimum **this** browser session ("this device") | ✅ *(after fix `022876b3`)* | [📷 sessions](evidence/auth-14-account-sessions.png) |
...
- **Journey verdict:** ☑ **PASS (step 2) / ⚠️ watch-item (step 1)** — Step 2 **fixed and prod-re-walked 2026-06-04**: fresh magic-link sign-in → `/account/identifiers` lists **`emrah.baysal@openova.io` — Magic-link email, Verified** + its Remove button ([#685](https://github.com/iogrid/iogrid/issues/685) full chain `7f26da1`: NextAuth `events.signIn` → BFF `POST /me/identifiers` → `IdentityService.EnsureIdentifier`, idempotent — existing accounts heal on next login). Original defect for the record: the endpoint probe mis-read as 404 (wrong path); the real root cause was that NextAuth authenticates outside identity-svc so **no identifier row was ever created** — masked by the "No identifiers bound" empty state. Step 1's "this device" row is **FIXED** (the watch-item predated the fix). Root cause was the same class as #685 — the web's session is a NextAuth row (Drizzle `session` table), which never reaches identity-svc, so identity-svc's `ListSessions` had no current row to flag. Fix shipped `022876b3` (Refs #685): a same-origin `/account/sessions/web-sessions` route surfaces the NextAuth sessions with `is_current` set by token-hash match against the live cookie, and `SessionsPanel` merges it with identity-svc sessions → the current browser row always renders (pill, no-revoke, pinned). Now covered both ends: the route's `is_current`/UA/no-token-leak/current-revoke-refusal by `web-sessions-route.test.ts` (5/5), the render by `sessions-panel.test.tsx`. (My #693 was a stale-duplicate filing — the gap was already closed.)
