...
| TC-20–25 | iOS app | Onboarding / connect / region / settings / top-up / live | **EXECUTED on the CI simulator, redesigned UI** (run 26925409479 + successors): flows **01–09 🟢 ALL PASS** w/ committed captures (09's first pass ever); 06 = sim-honest (`optional` — CONNECTED is device-only, [#574](https://github.com/iogrid/iogrid/issues/574) checklist); **10 exposed [#690](https://github.com/iogrid/iogrid/issues/690)** — a REAL app defect (fresh-install 401 + silent failure; D2 alert fix validating, D1 registration RPC next). jest 59✓. The chain found 1 app defect, 1 CI-infra bug (stale XCTest handle), and 3 test bugs — exactly what executing it was for. |
