...
| DEPLOY | Merged→prod path rolls every service to latest CI-green digest | ⚠️ **running, but prefix-collision freeze** | reroll cron installed/running (last tick 2026-06-24 exit=0), BUT `reroll-iogrid-deployments.sh:47-52` prefix-collision freezes `antiabuse-svc` on stale digest (latest `8e893046` never deploys). Tracking: **#822** |
