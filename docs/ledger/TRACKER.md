...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T21:45Z` 🟢🟢 **iogridd v0.1.1 MERGED + TAGGED**: PR #483 admin-merged (83ab6d6) — daemon 0.1.1 + 4 dispatch_bridge tracing lines. `git tag v0.1.1` pushed; installer-ci + daemon-ci running on the tag now (Sparkle .pkg + appcast.xml will publish on success). Hatice's auto-update polling already flipped on (config.toml `disabled=false` via SSH); her daemon will pull v0.1.1 on next manifest poll, log immediately reveals dispatch-bridge silent-failure root cause. Also closed inline: deduped stale Hatice provider row (808ce330 → status=deactivated). Filed #484 (follow-up: retire DEV_API_KEYS env hack, wire billing-svc ValidateApiKey RPC). vcard acked, holding smoke + Playwright spec for the moment workloads-svc sees dispatch open. |
