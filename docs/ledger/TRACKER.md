...
|  |  |
|---|---|
| Last refreshed | `2026-06-11T10:05Z` 🟢 hand-update (G1 REAL end-user bug fixed #721 → build 172; G2 isolation ADR + provisioning; G3 chain complete) |
| 🟢🟢🟢 2026-06-11 G1 — REAL end-user 'resolving peer' bug found + fixed (#721) → TestFlight build 172 | Founder repro on build 171 STILL failed at 'resolving peer'. Root cause (NOT the server — #709/#710 were necessary-not-sufficient): the mobile app's `ensureDeviceKeypair` used the App Group `group.io.iogrid.app`, but that entitlement is on the tunnel-EXTENSION target only — the MAIN-APP entitlement is commented out in `with-network-extension.ts` (Apple's API can't create App Groups). So `UserDefaults(suiteName:)` returned nil in the main app → the device sent an EMPTY key → bastion daemon log floods `session has no customer_wg_public_key yet; waiting` → no peer bound → handshake dropped → 'resolving peer'. Fix #721: keypair in the main app's standard UserDefaults; private half reaches the extension via providerConfiguration (read first, PacketTunnelProvider.swift:134) — no App Group needed. Build 172 built+signed+uploaded+assigned to hat.yil's vpn-beta. AWAITING founder on-device test (the one thing I can't do). Refs #701, #706, #709. |
