...
|  |  |
|---|---|
| Last refreshed | `2026-06-04T11:45Z` 🟢 cron-refresh: +1 commits since last TRACKER bump |
...
| 🎨 2026-06-04 #594 top-up price-transparency UX + 35 new mobile unit tests (founder mandate: 'UX artist/guru' + 'enrich the test cases') | The money screen made the user do the 100:1 \$GRID→USD math in their head: typed custom amounts showed no price, and the commit button said only 'Continue'. Now the custom field shows a live '≈ \$X.XX' as you type and the CTA reads 'Add N \$GRID / \$X.XX' — the amount + price on the control you authorize a real Ping charge with (on-brand: per-byte transparency). Backed by a new pure `src/lib/pricing.ts` (one source of truth for the ratio, defensive → never \$NaN/negative) with **6 tests**. Bundled with the session's other coverage: **35 new mobile unit tests total** — `connection-steps` (12, the #684 failure-honest state machine), `coordinator` (17, the #630 serialization seam + #690 503/credential paths), `pricing` (6). Mobile jest **59→94**. Maestro flow 09 re-validates the redesigned top-up (testIDs unchanged). Comprehensive CI run [26951465680](https://github.com/iogrid/iogrid/actions/runs/26951465680) in flight (`2653191a`, supersedes the mid-flight run cleanly — last queued mobile change, so it completes uninterrupted). |
