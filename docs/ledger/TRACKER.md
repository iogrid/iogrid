...
|  |  |
|---|---|
| Last refreshed | `2026-06-04T11:55Z` 🟢 hand-update: PriorityClass validated live; #689 BFF leg verified; 01–10 run building |
...
| 🔴🔴 2026-06-04 #682 now MULTI-PRODUCT: iogrid + ping deploy waves contend for the same slots; k8s housekeeping OutOfpods | Latest (03:25Z): while iogrid's wave drains, **ping's own deploy wave** (auth/transfer/ledger/user-service) started on the same node — each product's freed slots get consumed by the other, and a `kube-system` PVC-cleanup helper failed **`OutOfpods`** (k3s housekeeping can't run). Parked at 0: `admin`, `telemetry-svc` (→ /status dashboard dark). Reclaims exhausted this session: HPA fiction fixed (10→3), ~1.3Gi requests right-sized (7 svcs), CNPG standby reclaimed (`83d074c`). **Founder decision required:** max-pods runbook (1 SSH cmd — operator-gated by the no-solo-control-plane-restart rule) or node 2 (#652). Cost grows hourly: 2 surfaces dark, deploy latency for BOTH products, broken PVC cleanup. Public site + API healthy (edge/api 200). |
