...
|  |  |
|---|---|
| Last refreshed | `2026-06-01T00:20Z` 🟢 #562 closed (vpn-svc netpol egress to billing-svc+nats — was 401-faking every auth as "connection refused"). Self-served #560: minted dedicated smoke key via direct billing.api_key INSERT, stashed in repo secrets, verified 201 against live api. vpn-e2e-smoke now running first authed CI run. 19 PRs this session, 12 closures. Open: #532 (gated on #529 worker). |
