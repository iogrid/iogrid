...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T21:00Z` 🟢🟢 vcard relayed its actual smoke result via shepherd: TLS+SOCKS5+USERPASS handshake works (Traefik fix landed) but CONNECT rejected — vcard-prod key (efd20c9d) has geo_target=US vs hatice's Oman exit + allowed_categories=social-intel that providers schema doesn't carry. Minted fresh test key `iog_…f855` (id `1e543a45`) on the SAME vcard workspace with geo_target='' + allowed_categories='' — INSERTed inline into billing.api_key, delivered to vcard via chepherd.send_to_session. Awaiting LinkedIn-via-Mac-IP screenshot. Earlier: #479 CLOSED (Traefik 60s readTimeout fix in openova-private e7b238fa). Open: 5 (4 parked + #481 PR). |
