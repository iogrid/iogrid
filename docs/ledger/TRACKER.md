...
|  |  |
|---|---|
| Last refreshed | `2026-06-12T09:15Z` 🟢 hand-update (G1 build 177 VALID+READY_FOR_BETA_TESTING confirmed; G2 #742 live on Mac — build runs exit 0, no longer rejected; G3 full build→earnings chain shipped #741/#742/#743/#745, dashboard root cause traced to prod DB) |
| 🟢🟢🟢 2026-06-12T09:15Z G3 — build-completion chain SHIPPED + #742 PROVEN LIVE (build no longer "rejected") | Traced the dog-food build showing `rejected/-1` while it ran to exit 0: a daemon split-brain — the dispatch stream rejected the iOS build with `scheduler_paused` (Mac's idle scheduler paused) and that terminal rejection beat the poll path (#705) which ran it. **#742** (daemon: stream ignores IOS_BUILD, poll owns it) built + deployed live on Hatice's Mac → re-submitted build went `dispatched` not `rejected`, daemon ran exit 0. **#743** (workloads-svc: poll status callback now forwards to build-gateway — it never did, so builds stuck `dispatched` forever). **#741** (static identity carries a user, for the separate on-chain $GRID settle). All 3 MERGED. Refs #740, #705. |
| 🟢🟢 2026-06-12T09:15Z G3 — "Earnings 0 $GRID" root cause traced to prod DB + metering fix (#745) | The /provide card = `SUM(usage_event.cost_cents) WHERE provider_id`. Prod query: 397 rows ALL `BANDWIDTH_VPN`, every `cost_cents=0` because every session metered **0 bytes** (phantom sessions, downstream of G1 — pricing is correct, starved of input); **0 IOS_BUILD rows** (build metering Emitter was InMemory-only, no provider_id, no NATS publisher). **#745** (PR) wires it: thread provider_id forward→build, NATS metering emitter publishing billing's wire envelope (workload_type=IOS_BUILD, cost_cents=provider-share of billed minutes); NATS_URL set on build-gateway configmap. After deploy + a real build → IOS_BUILD usage_event with Hatice's provider + cost>0 → card moves off 0. Refs #744, #740. |
