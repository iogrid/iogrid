...
|  |  |
|---|---|
| Last refreshed | `2026-06-03T17:22Z` ✅ all 3 deep-UAT fixes MERGED (#678 #679 #680) — prod verifications rolling |
| ✅ 2026-06-03 deep-UAT fix TRIO merged — #678 + #679 + #680 (13/13 each) | The complete fix-set from the authenticated UAT is on main: **#678** 204-DELETE proxy crash (revoke + every DELETE now returns its real status; #676 closed), **#679** SubscriptionService.ListUsage implemented (customer /usage gets real metering; #675 closed), **#680** BFF api-keys List hides revoked rows (revoke is VISIBLY correct — was: even successful revokes looked failed because the "Active keys" refresh re-showed the revoked key with revoked_at dropped from the shape). Deploys via the reroll cron: web (204-fix) at the next tick; billing-svc + gateway-bff after their main coordinator builds. Final prod re-walks pending: revoke round-trip + kill the uat-test-key, /customer/usage rendering the 124 real rows, list hiding revoked. |
| ✅ 2026-06-03 #675 MERGED (PR #679, 13/13 green) + #680 list-residual in flight | **#675 CLOSED** — billing-svc now mounts SubscriptionService.ListUsage over the real `usage_event` data (124 prod rows will render on /customer/usage once billing-svc rolls). **#680** opened for the #676 residual: the BFF api-keys List passed revoked rows through to the UI's "Active keys" (shape drops `revoked_at`) — so even a SUCCESSFUL revoke looked failed after refresh; one-line filter + `ListHidesRevokedRows` test, CI running. Also resolved the UAT watch-item: provide/audit/stream 404 is **by design** (`no_provider` guard for unpaired callers, #313) — probe artifact, not a defect. Deploy sequencing: web (204-fix) rolls at the :20 tick → re-walk revoke; billing-svc rolls after coordinator-ci → re-walk usage. |
| ✅ 2026-06-03 #675 customer /usage IMPLEMENTED (PR #679) | The deep-UAT's 2nd real defect fixed: `/customer/usage` 501'd (masked as "$0.00") because the whole chain existed EXCEPT the last link — gateway-bff's `Clients.Billing.ListUsage` was fully wired (#635) but **billing-svc never mounted the SubscriptionService**. Implemented `store.ListUsageEvents` (workspace + half-open window + workload_type PREFIX filter so BANDWIDTH matches prod's `BANDWIDTH_VPN` rows, page subtotal) + `SubscriptionHandler.ListUsage` (30d default window, page cap 500, opaque offset token, size+1 sentinel, cents→micros ×10k) + route mount. go test ok (type-map incl. prod value + enum↔prefix round-trip), billing-svc+gateway-bff build/vet clean. Prod has 124 real usage_event rows that will render once deployed. Earlier this stretch: **#676 merged** (204-DELETE proxy crash, PR #678) → auto-closed. |
| ✅ 2026-06-03 DEEP authenticated UAT (signed in) — 2 real defects + #676 FIXED | Founder pushback: the prior UAT was surface-only. Signed in as a real user (magic-link to `emrah.baysal@openova.io`, read **read-only**) and walked the **authenticated** product: provider (dashboard/earnings/audit/staking, 200s), customer (workspace/billing 200, **minted a real API key** + one-time token reveal), wallet bind (renders, needs Phantom), account. **Doc: `docs/ledger/UAT-iogrid-2026-06-03.md`** (12 auth screenshots) + mobile cases grounded in the real Maestro 11-flow suite + jest (59✓). **Found 2 real customer defects only visible signed-in:** #675 usage 501 (masked as $0.00), #676 API-key **revoke broken**. **#676 ROOT-CAUSED + FIXED (PR #678):** the BFF proxy crashed re-emitting **204 No Content** (`new Response("",{status:204})` throws) → bare 500 → broke **every** 204-DELETE (revoke, wallet unbind, session revoke, account/workspace delete). One-block fix in `bff-proxy.ts` + regression test (7/7 green). Also corrected my over-filed #677 (workloads POST-only by design → closed invalid). |
