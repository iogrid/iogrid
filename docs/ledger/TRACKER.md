...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T20:45Z` 🟢🟢🟢 **PERMANENT FIX — Mac heartbeat broken-for-5-days CLOSED**: root cause was Traefik `websecure` entrypoint default `respondingTimeouts.readTimeout=60s` killing every daemon→coordinator bidi gRPC stream cluster-wide. Diagnosed by SSH-into-Mac via openova-bastion:2223 (user `emrah`, key `claude_offload`) reading `~/.iogrid/logs/iogridd.out.log` — found tonic h2 errors every 60s. Fix: openova-private commit `e7b238fa` adds `transport.respondingTimeouts.readTimeout/writeTimeout=0s, idleTimeout=3600s` to Traefik HelmChartConfig — survives chart upgrades. Live-verified: hatice's `provider-a7a93576` last_seen advancing every 5s for 3+ min straight (was 5 days silent). **#479 CLOSED**. Pinged vcard session to re-run LinkedIn smoke against real residential path. Open: 5 (4 parked + #481 PR). |
