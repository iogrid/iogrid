...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T21:30Z` 🟢 Option (b) shipped inline per shepherd nudge: **PR #483 open** — iogridd 0.1.1 + 4 dispatch_bridge tracing lines in spawn_live_dispatch's reconnect closure (entry / Channel::connect-FAILED / Channel::connect-OK / run_dispatch_stream-error). Linux fmt+clippy+test PASS, cross-platform release-build matrix pending. Also: flipped Hatice's iogridd config.toml `updater.disabled=true→false` via SSH; daemon logged `auto-update polling enabled`. **#481 MERGED** (d6a725a — iogridd diag CLI). Once #483 merges + I tag v0.1.1, installer-ci publishes signed .pkg + Sparkle appcast.xml → Hatice's daemon auto-updates → tracing reveals the dispatch silent failure. |
