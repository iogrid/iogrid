...
|  |  |
|---|---|
| Last refreshed | `2026-06-04T02:25Z` 🟢 hand-update: 13-svc cascade landed, memory right-size wave 2, telemetry crashloop root-caused |
...
| 🟡 2026-06-04 mobile UX pass 2 + flake-disambiguation run 26916652736 in-flight | Pass-2 commit `27d72e3`: the SIWA button's Apple-logo glyph was an EMPTY text node (capture proved it) → real  glyph (U+F8FF, HIG); the ~60%-dead sign-in screen got a privacy-assurance card (accent StatusDots: no logs / Apple hides email / works sans account) in the home-cards visual language. jest 59✓. The run doubles as the flow-05 flake disambiguation (it auto-cancelled run 26916431232 via concurrency group); Monitor armed → harvest pass-2 captures + full-chain (05–10) verdict. |
...
| 🔴 2026-06-04 admin.iogrid.org DOWN — node saturated on BOTH axes (#682 escalation) | After the web roll, `admin` cannot be restored: Pending on `Insufficient memory, Too many pods` even with 2 nominal slots free. The fleet no longer fits the single node. **Founder decision required:** execute the max-pods runbook (1 SSH cmd, ~30s control-plane blip) or provision node 2 (#652) or consciously trade a service out. The Pending pod self-heals when room appears. Public site + API healthy (edge/api 200). |
