...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T21:20Z` 🟡 Layer-3 caught: heartbeat bridge fully alive (`last_seen` 0.47s after Mac kickstart at 21:13:26Z), BUT dispatch bridge spawns + then silently fails to open RPC — no `dispatch stream: opening` log on daemon side, no `dispatch stream opened` log on workloads-svc side. Workloads-svc external IngressRoute IS wired correctly. **Filed #482** (severity/p1) with diagnostic plan (more logging in daemon/transport/lib.rs:1456-1466 + rebuild iogridd 0.1.1 + ship via auto-update). Pinged vcard with 2 founder-choice paths: (a) workloads-svc bypass hack, (b) iogridd rebuild. Test key minted earlier this turn (`iog_…f855`) remains valid. Open: 6 (4 parked + #481 PR + #482 in-progress). |
