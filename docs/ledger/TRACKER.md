...
|  |  |
|---|---|
| Last refreshed | `2026-06-03T12:05Z` ✅ #666 UAT standard → PR #667 (template + web/iOS samples); #646/#665 P23 re-investigated |
| ✅ 2026-06-03 #666 standardized UAT template + web/iOS samples → PR #667 | Adopted the OpenOva standardized end-user UAT format in `docs/ledger/` (mirrored canonical `UAT-TEMPLATE.md` + 2 samples). Shipped 3 docs: **UAT-TEMPLATE.md** (reusable, UI-only golden rule, web + native-mobile authoring rules), **UAT-SAMPLE-web-provider-customer.md** (REAL Playwright walk of iogrid.org — 6 live screenshots), **UAT-SAMPLE-mobile-vpn.md** (demonstration iOS walk; labeled placeholder evidence, real testIDs). Honest mix (web sign-in ⛔ inbox-gated; mobile top-up ⛔ external #665). Scoped `.gitignore` negation tracks `docs/ledger/evidence[-mobile]/`. docs-canonical green; all 16 evidence links resolve. **PR [#667](https://github.com/iogrid/iogrid/pull/667)** — independent reviewer in flight → close #666 on ACCEPT (founder merges PR async). |
