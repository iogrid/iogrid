...
|  |  |
|---|---|
| Last refreshed | `2026-06-03T12:45Z` ✅ real UAT walk → #668 fix (PR #669) + #670/#671 found + #671 fix (PR #672) |
| ✅ 2026-06-03 UAT-found defects cascade — #668 fixed, #670 filed, #671 E2E gate fixed | The real iogrid UAT walk surfaced a defect CLASS (cert-SAN subdomains with inert Gateway-API HTTPRoutes but no Traefik route → 404). **#668** (`/status`→`status.iogrid.org` 404) fixed in **PR #669** (repoint to working `api.iogrid.org/healthz`; local tsc clean). Subdomain audit found **#670** (security/privacy pages cite `docs.iogrid.org` 404 for PGP key + transparency report — repoint to live `iogrid.org/transparency`). Investigating #669's CI exposed **#671**: `web-e2e` was RED on main (no signal) — stale `login.spec.ts` hard-asserts the Google button #653 hides + `UntrustedHost` auth env. **Fixed in PR #672** (AUTH_TRUST_HOST=true + Google assertion matches config). One UAT walk → 1 fix + 2 new defects found + 1 dead CI gate restored. |
| ✅ 2026-06-03 REAL iogrid UAT walk (PR #667) + #668 status-404 fix (PR #669) | Filled the real iogrid UAT — `docs/ledger/UAT-iogrid-web-2026-06-03.md`, 7 web journeys walked LIVE vs iogrid.org (16 real screenshots): provider download, all 4 customer workloads (compute/proxy/gpu/ios-build w/ real pricing), VPN plans, magic-link sign-in (⛔ inbox-gated completion), auth-gating, trust surfaces. **Overall ⚠️ CONDITIONAL, no P0/P1.** iOS NOT WALKED (no build, #574). **Real P2 defect found → #668:** `/status` "Go to status dashboard" → `status.iogrid.org` returns raw `404` (subdomain has an inert Gateway-API HTTPRoute but the cluster routes via Traefik + no backend deployed). **Fixed in PR #669** (web-only, zero infra risk): `/status` now links only to the working `api.iogrid.org/healthz` + states the full dashboard is in progress; no dead button. Follow-up: stand up a real status dashboard (port StatusPageClient or Traefik IngressRoute + backend). |
