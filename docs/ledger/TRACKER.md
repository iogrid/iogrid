...
|  |  |
|---|---|
| Last refreshed | `2026-06-03T07:45Z` 🟢 cron-refresh: +5 commits since last TRACKER bump |
<<<<<<< Updated upstream
=======
| ✅ 2026-06-03 P23 AGGRESSIVE RE-EXAM #646 + #574 — split verdict | Exhausted every automatable path on the two "operator-gated" issues. **#646 Google OAuth: confirmed genuinely blocked-ext** — no gcloud/GCP creds on host, no reusable iogrid.org OAuth client (other repos' clients bound to sme/vcard domains → redirect_uri_mismatch), and the only programmatic mint (`gcloud iap oauth-clients create`) is IAP-locked with NO settable redirect URI (general web clients are Console-UI-only). Wiring proven 100% turnkey: callback 302→`error=Configuration` (placeholder signature), AUTH_URL+NEXTAUTH_URL both apex, all secret keys present — credential drop is the ONLY remaining step (runbook posted on issue). **#574 SPLIT:** privacy-nutrition-labels half **still cookie-gated** (re-probed today: all `appDataUsages*` + `dataTypes`/`categories` = HTTP 404; Apple still has no privacy API as of June 2026; `ASC_SESSION_COOKIE` NOT set → Playwright path parked). BUT external-beta half is **AUTOMATED & ALREADY IN REVIEW** — diagnose-beta-review today: privacyPolicyUrl set, betaAppReviewDetails fields now persist (#600 bug resolved), `POST betaAppReviewSubmissions` build 137 → 422 `ANOTHER_BUILD_IN_REVIEW` (a build is already in Apple's external beta queue via the ASC API key in GH secrets — no founder cookie). Recommend splitting #574: external-beta done-pending-Apple, park only privacy-labels. Confirmatory probe in PR #651. Refs #646 #574. |
