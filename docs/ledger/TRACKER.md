...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T22:25Z` 🟢 **#482 root cause IDENTIFIED + FIXED** — v0.1.1 dispatch_bridge tracing surfaced 60+ `reconnect-closure entered` log entries in <100ms with no `Channel::connect OK/FAILED` between them, proving the closure was being cancelled mid-`tracing::info!`. Diagnosis: `spawn_live_dispatch` returns `LiveDispatchHandle { cancel_tx, ... }`; supervisor was destructuring with `cancel_tx: _` (dropping watch sender) → `cancel.changed().await` returned `Err` on every poll → `tokio::select!` looped at sub-ms cadence cancelling the connect-future. Heartbeat side at L508 already pins its sender via `let _keep = cancel_tx;`. **v0.1.2 c5947dd shipped** — pinned `cancel_tx` on Supervisor struct for lifetime of `run(mut self)`. Tag pushed → daemon-ci building (run 26374418199). Next: auto-update or SSH-install onto Hatice's Mac → verify workloads-svc `dispatch stream opened` for cac83611 → ping vcard → LinkedIn smoke through residential IP → close #482. Open: 7 (4 parked + #481 PR + #482 active + #486 in-progress + #485 parked). |
