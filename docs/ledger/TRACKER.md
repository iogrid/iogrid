...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T22:10Z` 🟡 v0.1.1 .pkg installed on Hatice's Mac via SSH-tunnel scp+installer (manual path; auto-update was blocked by 404 manifest server #485 + installer-ci 403 on Windows publish #486). /usr/local/iogrid/iogridd version returns 0.1.1; binary cp'd to /Users/emrah/bin/iogridd to match LaunchAgent path. The kickstart cycle caused autossh tunnel churn (port 2223 keeps reconnecting then dying); heartbeat path back at 21:59:23Z (provider.heartbeat_recovered) but workloads-svc still reports zero dispatch-stream events. Verifying-via-SSH currently blocked by tunnel banner-timeout; cluster-side heartbeat IS flowing. Open: 7 (4 parked + #481 PR + #482 in-progress + #486 in-progress + #485 parked). |
