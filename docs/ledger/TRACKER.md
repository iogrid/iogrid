...
|  |  |
|---|---|
| Last refreshed | `2026-06-03T13:00Z` ✅ UAT defect cascade — 3 fix PRs (#669 #672 #673), E2E gate 21→clearing |
| ✅ 2026-06-03 UAT cascade — #668/#670 fixes + #671 E2E gate restored (3 PRs) | The real iogrid UAT walk → a defect class + a dead CI gate, all fixed: **#668** `/status`→`status.iogrid.org` 404 → **PR #669** (repoint to api healthz). **#670** security/privacy cite `docs.iogrid.org` 404 → **PR #673** (repoint to live iogrid.org/transparency + /security). **#671** `web-e2e` RED on main → **PR #672** in 3 iterative fixes: (1) `AUTH_TRUST_HOST=true` killed the UntrustedHost flood, (2) `login.spec` Google assertion matches #653, (3) `customer-auto-workspace` ambiguous quick-link selectors → `.first()`. After (1)+(2): 21 passed/4 failed (down from a flood); (3) clears the last 2 (×2 retries). One UAT walk surfaced + fixed 1 prod defect + 1 new prod defect + 1 dead CI gate. |
