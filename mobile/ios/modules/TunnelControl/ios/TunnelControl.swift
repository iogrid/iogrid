...
public class TunnelControlModule: Module {
  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("ensureDeviceKeypair") { (promise: Promise) in
      guard let defaults = UserDefaults(suiteName: Self.statsAppGroup) else {
        promise.reject("NO_APP_GROUP", "App Group \(Self.statsAppGroup) unavailable")
        return
      }
    ...
    AsyncFunction("startTunnel") { (config: TunnelConfig, promise: Promise) in
      Self.loadManager { manager, _ in
        ...
        // Inject the device's persistent WG private key (App-Group shared,
        // generated by ensureDeviceKeypair) so the extension uses the SAME
        // keypair whose PUBLIC half the app registered with vpn-svc — this
        // is what lets the provider accept the device as a peer and the WG
        // handshake complete. (#701) The private key stays native; JS never
        // sees it.
        if let defaults = UserDefaults(suiteName: Self.statsAppGroup),
           let priv = defaults.string(forKey: Self.devicePrivKeyDefaultsKey),
           !priv.isEmpty {
